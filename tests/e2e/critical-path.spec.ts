...
test.describe("Authentication", () => {
  ...
  test("should navigate to register page from login", async ({ page }) => {
    await page.goto(`${BASE_URL}/login`);
    await page
      .locator('a[href*="register"], a[href*="signup"], a[href*="sign-up"]')
      .first()
      .click();
...
test.describe("Project CRUD", () => {
  ...
  test("should create a new project", async ({ page }) => {
    ...
    await page
      .locator("button, a")
      .filter({ hasText: /new project|create|nouveau/i })
      .first()
      .click();
    await page
      .locator('input[name="title"], input[placeholder*="title"], input[placeholder*="titre"]')
  ...
  test("should list user projects", async ({ page }) => {
    await page.goto(`${BASE_URL}/projects`);
    await expect(page.locator('[data-testid="project-card"], .project-card, article')).toHaveCount(
      1
    );
  });
  ...
  test("should edit project title", async ({ page }) => {
    ...
    await page
      .locator('[aria-label*="edit"], [title*="edit"], button:has-text("Edit")')
      .first()
      .click();
...
test.describe("AI Story Generation", () => {
  test("should have a generate button on project page", async ({ page }) => {
    ...
    await expect(
      page.locator(
        'button:has-text("Generate"), button:has-text("AI"), button:has-text("Assistant")'
      )
    ).toBeVisible();
  });
  ...
  test("should display error on AI failure", async ({ page }) => {
    ...
    const errorVisible = await page
      .locator('.error, .toast-error, [role="alert"]')
      .isVisible()
      .catch(() => false);
    const promptVisible = await page
      .locator("text=API key, text=configure, text=settings")
      .isVisible()
      .catch(() => false);
...
test.describe("Stripe Payments", () => {
  ...
  test("should redirect to Stripe Checkout on subscribe click", async ({ page }) => {
    ...
    const subscribeBtn = page
      .locator('a[href*="checkout"], button:has-text("Subscribe"), button:has-text("Get Started")')
      .first();
