...
import { LanguageBuilder } from "@/components/world/language-builder";
...
export default function LanguagePage() {
  ...
  useEffect(() => {
    fetch("/api/world/encyclopedia?category=language")
      .then((r) => r.json())
      .then(setEntries)
      .catch(() => {})
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-fg/60 text-sm mt-1">
            Construct languages, phonologies, and scripts for your world.
          </p>
        </div>
        <button
      ...
      <Link href="/world/encyclopedia/language" className="text-sm text-fg/40 hover:text-brand inline-block">
        View all language encyclopedia entries
      </Link>

      {showBuilder && (
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold">Create Language</h2>
            <button
          ...
          <LanguageBuilder onSaved={handleSaved} />
        </Card>
      )}
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center">
          <p className="text-fg/40">No languages defined yet.</p>
          <button
            onClick={() => setShowBuilder(true)}
        ...
        <div className="grid gap-3">
          {entries.map((entry: any) => (
            <Link
              key={entry.id}
              href={`/world/encyclopedia/language/${entry.id}`}
            >
