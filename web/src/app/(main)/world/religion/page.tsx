...
import { ReligionBuilder } from "@/components/world/religion-builder";
...
export default function ReligionPage() {
  ...
  useEffect(() => {
    fetch("/api/world/encyclopedia?category=religion")
      .then((r) => r.json())
      .then(setEntries)
      .catch(() => {})
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold">Religions &amp; Beliefs</h1>
          <p className="text-fg/60 text-sm mt-1">
            Define the religions, deities, and belief systems in your world.
          </p>
        </div>
        <button
      ...
      {showBuilder && (
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold">Create Religion</h2>
            <button
          ...
          <ReligionBuilder onSaved={handleSaved} />
        </Card>
      )}
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center">
          <p className="text-fg/40">No religions defined yet.</p>
          <button
            onClick={() => setShowBuilder(true)}
