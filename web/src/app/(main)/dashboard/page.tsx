...
import { BookOpen, Users, MessageSquare, Target, BarChart3, Trophy, Flame, Bell, Globe } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const sections = [
    { label: "Writing", items: [
      { href: "/projects", label: "My Projects", desc: `${projectCount} project${projectCount !== 1 ? "s" : ""}`, icon: BookOpen },
      { href: "/projects/new" in String ? "/projects" : "/projects", label: "New Project", desc: "Start writing", icon: BookOpen },
    ...
    { label: "World Building", items: [
      { href: "/world", label: "World Overview", desc: "Characters, locations, timeline", icon: Globe },
      { href: "/world/encyclopedia", label: "Encyclopedia", desc: "Lore, magic, cultures", icon: BookOpen },
      { href: "/world/gallery", label: "Image Gallery", desc: "Visual references", icon: Globe },
    ]},
    { label: "Social", items: [
      { href: "/messages", label: "Messages", desc: unreadMessages > 0 ? `${unreadMessages} unread` : "No new messages", icon: MessageSquare },
      { href: "/notifications", label: "Notifications", desc: unreadNotifications > 0 ? `${unreadNotifications} unread` : "All caught up", icon: Bell },
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10 space-y-8">
      <div>
        ...
        <p className="text-fg/60 mt-1">Welcome back{user.email ? `, ${user.email.split("@")[0]}` : ""}. Keep the ink flowing.</p>
        {streakCount > 0 && (
      ...
      {sections.map((section) => (
        <div key={section.label}>
          <h2 className="text-sm font-bold text-fg/40 uppercase tracking-wide mb-3">{section.label}</h2>
